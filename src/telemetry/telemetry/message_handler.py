...
class AbstractMessage(ABC):
    ...
    def __init__(self, data: bytes):
        """
        Initializes the message with the given bytes. The header must already be stripped
        """
        pass
...
class RemoteMovementIntent(AbstractMessage):
    ...
    The first byte represents the drive (byte value of 0 represents -1.0 drive, and value of 255 represents 1.0 drive)
...
def parse_message(message: bytes):
    """
    Parses the given byte stream into a valid message
    """
    header = message[0]
    message = message[1::]
